...
- f1f5a95: remove eslint `missing-image-loader` rule
  ...
  the `missing-image-loader` rule is no longer needed since `@khulnasoft/next-on-pages`
  version 1.3.0 as basic support for the `Image` component has been introduced
  (in the following PR: https://github.com/khulnasoft/next-on-pages/pull/357), therefore there isn't
