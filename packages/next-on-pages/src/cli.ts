...
program
	.description(`@khulnasoft/next-on-pages CLI v.${nextOnPagesVersion}`)
	.allowExcessArguments(false)
	.configureHelp({
		commandUsage: () => '@khulnasoft/next-on-pages [options]',
	})
	.helpOption(undefined, 'Shows this help message')
	.addHelpText(
		'after',
		'\n' +
			'GitHub: https://github.com/khulnasoft/next-on-pages\n' +
			'Docs: https://developers.khulnasoft.com/pages/framework-guides/deploy-a-nextjs-site',
	)
	.addOption(
	...
	.addOption(
		new Option(
			...
			// when turbopack is introduces (see https://github.com/khulnasoft/next-on-pages/pull/208/files#r1192279816)
			.hideHelp(),
	)
	.option(
		'-s, --skip-build',
		'Skips the application Vercel build process (only runs the @khulnasoft/next-on-pages build logic)',
	)
	.option(
...
export function cliError(
	...
	if (showReport) {
		cliError(
			'Please report this at https://github.com/khulnasoft/next-on-pages/issues.',
			{ fromVercelCli },
		);
	}
}
...
export async function printEnvInfo(): Promise<void> {
	...
	const envInfoMessage = dedent(
		...
		System:
			...
			`
		Relevant Packages:
			@khulnasoft/next-on-pages: ${nextOnPagesVersion}
