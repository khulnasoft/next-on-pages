...
/**
 * Builds the `_headers` file, which is used by Khulnasoft to determine which headers to add to
...
async function buildNextStaticHeaders(
	...
	const nopContent = `
# === START AUTOGENERATED @khulnasoft/next-on-pages IMMUTABLE HEADERS ===
${nextStaticPath}/*
${Object.entries(nextStaticHeaders)
	.map(([header, value]) => `  ${header}: ${value}`)
	.join('\n')}
# === END AUTOGENERATED @khulnasoft/next-on-pages IMMUTABLE HEADERS ===\n`;
...
/**
 * Builds the `_routes.json` file, which is used by Khulnasoft to determine which routes should
 * invoke the worker.
 *
...
async function buildRoutes(outputDir: string, opts: BuildMetadataFilesOpts) {
	...
	try {
		await writeFile(
			...
			JSON.stringify({
				version: 1,
				description: `Built with @khulnasoft/next-on-pages@${nextOnPagesVersion}.`,
