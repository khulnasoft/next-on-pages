...
- ad4ca02: Convert dynamic requires for `khulnasoft:*` built-ins to import statements.
- ad4ca02: Mark `khulnasoft:*` as external for esbuild.
...
- 5712c57: Add new `getRequestContext` utility
  ...
  to their Khulnasoft `env` but also to their `cf` and `ctx` objects
...
- 06de52e: add new `setupDevPlatform` while deprecating `setupDevBindings`
  ...
  the `setupDevBindings` function (see: https://github.com/khulnasoft/next-on-pages/tree/main/internal-packages/next-dev#how-to-use-the-module)
  ...
  - consistent with the newly introduced `getBindingsProxy` utility (which is actually being used here under the hood)
    (https://developers.khulnasoft.com/workers/wrangler/api/#getbindingsproxy)
...
- 20d0e0d: Fix `_not-found` functions only for applications using the App Router
  ...
  In https://github.com/khulnasoft/next-on-pages/pull/418 we introduced a workaround
...
- 61d0a2f: print warning in case developers are using a different package manager to run @khulnasoft/next-on-pages
...
- bf57ec9: Fix prerendered `next.config.js` `basePath` root infinite redirect due to Khulnasoft Pages handling of `/{path}/index`.
...
- ee6b7dc: add user-agent header to outgoing fetch requests
  ...
  see: https://github.com/khulnasoft/next-on-pages/issues/376#issuecomment-1628416988)
...
- cb7ab43: Support for images via `/_next/image`, falling back to the raw image URL when image resizing is not available.
  ...
  Due to limitations with Khulnasoft Pages, it is not currently possible to send requests through image resizing.
...
- 505be1e: avoid extracting chunks when unnecessary
  ...
  As part of our lazy loading implementation (see https://github.com/khulnasoft/next-on-pages/blob/main/docs/technical/lazy-loading.md)
...
- 3e2dad8: use and rely on AsyncLocalStorage
  ...
  this is a breaking change since moving forward all app running using @khulnasoft/next-on-pages must
...
- d81c2e3: add pnpm files to ignored list for watch mode
  ...
  - `.pnpm-store`: to prevent potential infinite loops (https://github.com/khulnasoft/next-on-pages/issues/212)
