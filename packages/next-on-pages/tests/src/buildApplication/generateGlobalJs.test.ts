...
describe('generateGlobalJs', async () => {
	describe('AsyncLocalStorage', () => {
		...
		test('create an AsyncLocalStorage and set it as a proxy to the global khulnasoft request context variable', async () => {
			const globalJs = generateGlobalJs();
			expect(globalJs).toContain(
			...
			const proxyRegexMatch = globalJs.match(
				/globalThis\[Symbol\.for\('__khulnasoft-request-context__'\)\] = [\S\s]*?new Proxy\(([\s\S]+)\)[\s\S]+/,
			);
